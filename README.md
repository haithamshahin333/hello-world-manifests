# GitOps Demo and Manifest Repo for the Hello-World App
...
1. Run the command `touch .env` to create a .env file in the `./scripts` directory and fill in the following environment variables. These will be needed to deploy your AKS cluster and Argo CD.
...
2. Run the following commands from the `./scripts` folder where your `.env` file should be located:
...
1. Set your environment with your .env variables as you did previously by running the commands below from the `./scripts` directory:
...
2. Run the `./deploy-argo.sh` script (assumes that kubectl is still set since you deployed your aks cluster previously).
